...
CREATE TABLE GEA_USER(
    ...
    FOREIGN KEY (user_meth_name) REFERENCES GEA_METHODOLOGY(meth_name)
);
CREATE TABLE GEA_SCHEDULE(
    ...
    FOREIGN KEY (sche_user_email) REFERENCES GEA_USER(user_email)
);
CREATE TABLE GEA_SUBJECT(
    ...
    FOREIGN KEY (sub_sche_title) REFERENCES GEA_SCHEDULE(sche_title),
    FOREIGN KEY (sub_sche_user_email) REFERENCES GEA_SCHEDULE(sche_user_email)
);
CREATE TABLE GEA_TOPIC(
	top_name VARCHAR(45) PRIMARY KEY NOT NULL,
    top_user_email VARCHAR(45) NOT NULL,
    FOREIGN KEY (top_user_email) REFERENCES GEA_USER(user_email)
);
CREATE TABLE GEA_CHAT(
    ...
    FOREIGN KEY (chat_top_name) REFERENCES GEA_TOPIC(top_name),
    FOREIGN KEY (chat_top_user_email) REFERENCES GEA_TOPIC(top_user_email)
);
